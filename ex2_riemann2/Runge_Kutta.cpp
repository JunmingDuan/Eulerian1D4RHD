...
#pragma omp parallel for num_threads(Nthread)
  for(u_int i = 0; i < N_x; ++i) {
    Con[i] = 0.75*Con_n[i]
        + 0.25*Con[i] - 0.25*dt*(FLUX[i+1] - FLUX[i]) / (mesh1[i+1]-mesh1[i]);
    Pri[i] = Con2Pri(Con[i], Gamma[i]);
  }
  update_cs(cs);
  //stage 3
...
#pragma omp parallel for num_threads(Nthread)
  for(u_int i = 0; i < N_x; ++i) {
    Con[i] = 1./3*Con_n[i]
        + 2./3*Con[i] - 2./3*dt*(FLUX[i+1] - FLUX[i]) / (mesh[i+1]-mesh[i]);
    Pri[i] = Con2Pri(Con[i], Gamma[i]);
  }
  update_cs(cs);
...
#pragma omp parallel for num_threads(Nthread)
  for(u_int i = 0; i < N_x; ++i) {
    Con[i] = 0.75*Con_n[i]
        + 0.25*Con[i] - 0.25*dt*(FLUX[i+1] - FLUX[i]) / (mesh[i+1]-mesh[i]);
    Pri[i] = Con2Pri(Con[i], Gamma[i]);
  }
  update_cs(cs);
  //stage 3
